...
No OpenAI, Claude, Gemini or Ollama call; no Musebook; no Web3; no persistent
memory; no multiple agents; no autonomous levelling logic. The Brain contract is
provider-independent, so a richer policy or an LLM can be substituted without
touching the observation boundary, the validator or the trace.
