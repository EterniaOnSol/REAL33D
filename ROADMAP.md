...
8. `TWO-CLIENT-VERTICAL-SLICE-001` (`PASS`): the original `Tibia.exe` and Protocol772Core held two distinct characters in one Fusion32 world for 19 minutes. No opcode was added; the run instead corrected two known-creature lifetime errors and established that a listening-only client is disconnected after 90 rounds. Independent repetition and retained screenshots remain pending.
...
9. Run both clients simultaneously with two accounts. Done at the protocol level by `TWO-CLIENT-VERTICAL-SLICE-001`: visibility in both directions, tracked movement, client-initiated movement observed from the other client, and blocked movement caused by the other player. Still outstanding for `CERTIFIED`: an Unreal client on the B side, retained sanitized screenshots, and independent repetition.
