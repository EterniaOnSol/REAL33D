...
## UNREAL-WIDE-WORLD-001 source gate - 2026-09-21

- Agent/role: Codex, Unreal world infrastructure. Branch/worktree: main, C:/Users/dell/Desktop/fusion32. Starting and ending commit: 4637437a495f22134af5206b93c342c97589be49; no commit or push in this task.
- Objective: separate Fusion32's authoritative 18×14 live WorldState from a configurable, sector-streamed, static visual world, preserving V08 artwork and server/protocol semantics. The operator explicitly said to stop and document if a requested object category cannot be reconstructed faithfully.
- Inspection: reference/game/src/config.cc::ReadConfig, map.cc::ReadMapConfig/InitMap/LoadMap/LoadSector/LoadObjects/InitSector, reader.cc::ProcessLoadSectorOrder, sending.cc::SendFullScreen/SendItem, objects.hh::ObjectType::getDisguise, real sanitized-runtime .sec files, objects.srv, and the frozen V08 runtime catalogue.
- Discovery: the server loads writable game/state/map/*.sec; game/reference/origmap/*.sec is the pristine refresh baseline. Sectors are 32×32×Z; tiles carry recursive content and attributes. Source TypeID can differ from visible TypeID because SendItem applies DisguiseTarget. At Thais (32330,32226), radius-64 baseline coverage includes three stair tiles at z=6 whose source ID 451 is sent visibly as ID 469; V08 has no mesh for 469. Counts rise to 9 at radius 96 and 17 at radius 128. This is a visual-category blocker, not a wire or map-coordinate discovery.
- Changes: reverted this task's experimental FOV-only source and V08 launcher edits; preserved the previous uncommitted player-vitals work. Added only scripts/client/audit_wide_world_sources.py, evidence/clientcore/UNREAL-WIDE-WORLD-001.md, and this status documentation. No static streamer, map Actors, interaction, camera, server, protocol, meshes, aliases, materials, pivots, gallery, or V08 notes were changed. The closing V08 evidence JSON from the aborted experimental launch was restored to its prior clean state.
- Tests/evidence: Unreal compiled successfully for the earlier FOV-only experiment, which was then reverted and is not a wide-world test. The read-only source audit ran at radii 32/64/96/128 over floors 0..7: 47/93/144/199 sectors, with missing visible TypeID 469 counts 0/3/9/17. Exact commands, hashes, positions, and trace are in evidence/clientcore/UNREAL-WIDE-WORLD-001.md. No runtime sector crossing, FPS, memory, cache, seam, or reconciliation test was run; no PASS claim.
- Status/risk: UNREAL-WIDE-WORLD-001 = BLOCKED; VISUAL-FULL-CATALOG-INGEST-TEST-001 = STANDBY. A regex coverage audit is not a strict sector parser. The mutable server map may differ from origmap; distant presentation would be STATIC_BASELINE_ONLY. The earlier launch may have appended to the already-modified V08 movement journal; preserve it for review.
- Next task: obtain a source-backed visible stair appearance for TypeID 469 without altering frozen V08 assets, or obtain an operator decision to lift that freeze. Then build a strict .sec parser with fail-closed unknown attributes, bounded async 32×32×Z sector cache, game-thread instance creation, complete live-window suppression including empty positions, and baseline-only labels. Only after that compare fixed-coordinate protocol-only versus wide radius 64 and measure 32/64/96/128, transitions, memory, actor/instance counts, sector latency, and frame time. Do not call the current milestone PASS.
- Final build after FOV reversal: REAL33DEditor Win64 Development, Result Succeeded. The build validates restored source/binary alignment only.