...
| Movement | Source traced: `Move` ordering, `NotifyGo`, `CGoDirection`, the refusal paths | Cardinal walk/turn/stop commands and `MOVE_CREATURE`/`SNAPBACK`/`MESSAGE` implemented; diagonals and `GO_PATH` deliberately unexposed | No | `MOVEMENT-772-001` and `TWO-CLIENT-VERTICAL-SLICE-001` PASS; client-initiated steps observed from the original client, and three refusals caused by another player blocking the destination field left the world state untouched | No | IN_PROGRESS |
| Creatures | Source traced for the three descriptor forms, the known-creature table and its lifetime, creature relocation and the six attribute updates | Descriptors, the known-creature mirror, creature moves and the 140-145 attribute updates decoded and applied; the mirror now retains entries exactly as `TConnection::KnownCreatureTable` does | No | `INITIALWORLD-772-001`, `MOVEMENT-772-001`, `PLAYERSTATE-772-001` and `TWO-CLIENT-VERTICAL-SLICE-001` PASS; a second real player was tracked through appearance, movement, scroll-out, return, disconnect and relog with no phantoms | No | IN_PROGRESS |
