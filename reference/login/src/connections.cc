...
static TConnection *AssignConnection(int Socket, uint32 Addr, uint16 Port){
	...
	if(ConnectionIndex != -1){
		...
		Connection->RemotePort = (int)Port;
...
static void CheckConnectionInput(TConnection *Connection, int Events){
	...
	while(true){
		...
		if(Connection->RWPosition >= ReadSize){
			ASSERT(Connection->RWPosition == ReadSize);
			if(Connection->RWSize == 0){
#if ALLOW_LOCAL_PROXY
				if(Connection->Buffer[0] == 0x0D && Connection->Buffer[1] == 0x0A){
					ASSERT(sizeof(Connection->Buffer) >= 28);
					Connection->RWSize = 28;
					Connection->ReadingProxyHeader = true;
				}else if(Connection->Buffer[0] == 0xD9 && Connection->Buffer[1] == 0xFF){
					ASSERT(sizeof(Connection->Buffer) >= 6);
					Connection->RWSize = 6;
					Connection->ReadingProxyHeader = true;
				}else
#endif
...
#if ALLOW_LOCAL_PROXY
			else if(Connection->ReadingProxyHeader){
				uint8 *SourceAddr = NULL;
				if(Connection->Buffer[0] == 0x0D && Connection->Buffer[1] == 0x0A){
					ASSERT(ReadSize == 28);
...
static void CheckConnectionOutput(TConnection *Connection, int Events){
	...
	while(true){
		...
		// NOTE(fusion): Both login and status protocols will serve a single
		// request so we always close the connection after it is served.
		Connection->RWPosition += BytesWritten;
		if(Connection->RWPosition >= Connection->RWSize){
			ASSERT(Connection->RWPosition == Connection->RWSize);
			CloseConnection(Connection);
			break;
		}
	}
