...
Current phase: `PHASE 1 - CLIENT CORE` (`IN_PROGRESS`; classic functional baseline retained, Transport, Crypto, Login, Game Login and Initial World `PASS`)
...
Protocol: `IN_PROGRESS`; Transport, Crypto, Login, Game Login and the `FULLSCREEN` initial world snapshot are `PASS` for deterministic fixtures plus bounded local synthetic-account smoke; all other server commands remain recognized by name but unparsed
Client core: Unreal-independent C++17 TCP/framing/crypto/Login/Game Login/Initial World components implemented; retained and new suites pass in normal and ASan/UBSan WSL builds; native Windows build remains unverified
...
Latest functional tests: classic baseline IDs, `CLIENTCORE-TRANSPORT-772-001`, `CLIENTCORE-CRYPTO-772-001`, `CLIENTCORE-LOGIN-772-001`, `CLIENTCORE-GAMELOGIN-772-001` and `INITIALWORLD-772-001` are `PASS`
...
Next task: `MOVEMENT-772-001` (plan/source-trace only; do not start automatically). The map deltas it needs - `SV_CMD_ROW_*`, `FIELD_DATA`, `ADD_FIELD`, `CHANGE_FIELD`, `DELETE_FIELD`, `MOVE_CREATURE`, `FLOOR_UP`, `FLOOR_DOWN` - reuse the tile and skip encoding already decoded here.
